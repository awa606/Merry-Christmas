...
// Reusable objects for matrix and color calculations to avoid GC overhead
...
const Ornaments: React.FC<OrnamentsProps> = ({ progressRef }) => {
  ...
  useFrame((state) => {
    ...
    const updateInstances = (mesh: THREE.InstancedMesh | null, subData: OrnamentData[]) => {
      ...
      subData.forEach((d, i) => {
        ...
        // Physics weight influence on pathing during assembly/shatter
  ...
  return (
    <group>
      {/* Ball Ornaments */}
      <instancedMesh ref={ballMeshRef} args={[null!, null!, ballData.length]} castShadow>
        <sphereGeometry args={[1, 16, 16]} />
        <meshStandardMaterial metalness={0.9} roughness={0.1} />
      </instancedMesh>

      {/* Gift Boxes */}
      <instancedMesh ref={giftMeshRef} args={[null!, null!, giftData.length]} castShadow>
        <boxGeometry args={[1, 1, 1]} />
        <meshStandardMaterial metalness={0.6} roughness={0.4} />
      </instancedMesh>

      {/* Tree Lights */}
      <instancedMesh ref={lightMeshRef} args={[null!, null!, lightData.length]}>
